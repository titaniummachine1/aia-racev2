...
def build() -> Graph:
    ...
    denom_safe = g.add("ClampFloat")   # NEVER /0: straights = huge R = no brake
    g.connect(denom, "Float1", denom_safe, "Float1")
    g.connect(F(0.02), "Float1", denom_safe, "Float2")
    g.connect(F(1000000000.0), "Float1", denom_safe, "Float3")
    radius = g.add("DivideFloats")                             # R = abc/(2|cross|)
    g.connect(abc, "Float1", radius, "Float1")
    g.connect(denom_safe, "Float1", radius, "Float2")
    ...
    hitmask = g.add("MultiplyFloats")      # mask: no reading = never brake (the
    g.connect(hit, "Float1", hitmask, "Float1")   # traumatized-car fix)
    g.connect(F(10.0), "Float1", hitmask, "Float2")
    hitmask_c = g.add("ClampFloat")
    g.connect(hitmask, "Float1", hitmask_c, "Float1")
    g.connect(F(0.0), "Float1", hitmask_c, "Float2")
    g.connect(F(1.0), "Float1", hitmask_c, "Float3")
    brake_wall_m = g.add("MultiplyFloats")
    g.connect(brake_wall, "Float1", brake_wall_m, "Float1")
    g.connect(hitmask_c, "Float1", brake_wall_m, "Float2")
    ...
    g.connect(brake_wall_m, "Float1", bsum, "Float2")
    bdf = g.add("SubtractFloats")
    g.connect(brake, "Float1", bdf, "Float1")
    g.connect(brake_wall_m, "Float1", bdf, "Float2")
    babs = g.add("AbsFloat")
